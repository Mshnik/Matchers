...
import com.redpup.matchers.proto.CombiningMatcher as ProtoCombine
...
class CombiningMatcherTest {
  ...
  fun `COMBINE_ALL evaluates like an AND logic configuration`() {
    ...
    assertTrue(KMatcher.compile(allTrueProto).match("arbitrary"))
    assertFalse(KMatcher.compile(mixedProto).match("arbitrary"))
  }
  ...
  fun `COMBINE_NONE evaluates like a NOR logic configuration`() {
    ...
    assertTrue(KMatcher.compile(allFalseProto).match("arbitrary"))
    assertFalse(KMatcher.compile(mixedProto).match("arbitrary"))
  }
}
